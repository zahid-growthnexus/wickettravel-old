...
/**
 * "Parents Tickets" lead-capture section. Wicket Travel personally matches
 * families who need someone to accompany an elderly parent on a flight with
 * travellers already going the same way. One form with a toggle at the top:
 *   • Traveller  — "I can help a parent"      → enquiry_type: "traveller"
 *   • Requester  — "I need help for my parent" → enquiry_type: "requester"
 * The toggle swaps the conditional fields; shared fields stay put so nothing
 * is lost when switching. Submits JSON to the portal via our same-origin
 * /api/parent-ticket relay (the portal's CORS allowlist doesn't cover every
 * domain this site runs on). Success shows the returned #PT- reference.
 */

const EASE = [0.22, 1, 0.36, 1] as const;

// Same-origin relay (app/api/parent-ticket) that forwards to the portal.
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (res.ok && result.ok) {
        ...
        setSubmitError(
          result.error ?? "Your enquiry could not be submitted."
        );
      }
    } catch {
                ...
                {submitted ? (
                  ...
                  >
                    ...
                    <p className="t-small mx-auto mt-3 max-w-sm text-slate-600">
                      Thank you, {data.fullName.split(" ")[0] || "friend"} — our
                      team will be in touch shortly on{" "}
                    ...
                    {submitError && (
                      ...
                      >
                        ...
                        <div className="text-sm leading-relaxed text-red-800">
                          ...
                          <p className="mt-1">
                            {submitError} Everything you&apos;ve entered is still
                            here — please try again, or call / WhatsApp us on{" "}
                            <a
                    ...
                    <div className="mt-8 border-t border-slate-100 pt-6">
                      ...
                      <div className="mt-5 flex items-start gap-3 rounded-xl border border-navy-100 bg-navy-50/70 p-4">
                        ...
                        <p className="text-xs leading-relaxed text-navy-800">
                          ...
                          — your details are used only to arrange your match.
